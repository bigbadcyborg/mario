...
void Obstacle::drawObstacle(SDL_Plotter &x, int R, int G, int B, int r, int g,
                            int b) {
...
// draws an obstacle
void Block::drawObstacle(SDL_Plotter &x,
                         int a[36][36]) // SSS this is changed plz upload
{
  int r, g, b;
...
bool Sprite::collidesWith(Obstacle &p) {
  logic::Rect spriteBounds{posX, posY, width, height};
  logic::Rect obstacleBounds{p.getPosX(), p.getPosY(), p.getWidth(),
                             p.getHeight()};
  return logic::intersectsInclusive(spriteBounds, obstacleBounds);
}
...
bool Sprite::collidesWith(Block &p) {
  logic::Rect spriteBounds{posX, posY, width, height};
  logic::Rect blockBounds{p.getPosX(), p.getPosY(), p.getWidth(),
                          p.getHeight()};
  return logic::intersectsExclusive(spriteBounds, blockBounds);
}
...
bool Sprite::collidesWith(Sprite &x) {
  logic::Rect spriteBounds{posX, posY, width, height};
  logic::Rect otherBounds{x.getPosX(), x.getPosY(), x.getWidth(),
                          x.getHeight()};
  return logic::intersectsExclusive(spriteBounds, otherBounds);
}
...
void Sprite::drawMarioPixelArt(SDL_Plotter &x, int a[40][40],
                               bool f) { // S original function
  if (f) {
    for (int col = 0; col < width; col++) {
      for (int row = 0; row < height; row++) {
        if (a[row][col] == 1) {
...
bool Sprite::touchesBottomOf(Obstacle &x) {
  if ((posY == (x.getPosY() + x.getHeight()) + 1) &&
      ((posX + width > x.getPosX()) || posX < x.getPosX() + x.getWidth())) {
    return true;
  } else
    return false;
}
...
bool collidedBottomObstacle(Sprite &x, Obstacle list[], int size) {
  ...
  for (int i = 0; i < size; i++) {
    if (x.collidesWith(list[i]) &&
        (x.getPosY() == list[i].getPosY() + list[i].getHeight())) {
      collidedBottom = true;
    }
  }
...
bool collidedTopObstacle(Sprite &x, Obstacle list[], int size) {
  ...
  for (int i = 0; i < size; i++) {
    if (x.collidesWith(list[i]) &&
        (x.getPosY() + x.getHeight() == list[i].getPosY())) {
      collidedTop = true;
    }
  }
  return collidedTop;
...
void checkPos(Sprite list[], int size) {
  for (int i = 0; i < size; i++) {
    if (i % 3 == 0) {
      ...
      else if (list[i].getPosX() == 0 &&
               list[i].getPosY() + list[i].getWidth() > 10) { // S
        list[i].setPosX(960);
      }
      // go one side to other
      ...
      // if sprite is at the bottom right corner reposition to top left corner
      else if (list[i].getPosX() + list[i].getWidth() == 960 &&
               list[i].getPosY() >= 535) {
        list[i].setPosX(0);
        list[i].setPosY(0);
      }
  ...
  if (num == 0) {
    for (int row = 0; row < 8; row++) {
      for (int col = 0; col < 10; col++) {
        g.plotPixel((x + col), (y + row), box[row][col].r, box[row][col].g,
                    box[row][col].b);
      }
    }
