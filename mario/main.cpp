...
int main(int argc, char **argv) {
  ...
  logic::GameState gameState{
      score, marioLives, {level, numEnemies, deadEnemies}};
  ...
  Sprite coins[15] = {coin1,  coin2,  coin3,  coin4,  coin5,
  ...
  for (int i = 0; i < 18; i += 2) {
    for (int p = 0; p < 22; p += 2) {
      scoreBoard >> scoreDig;
      if (scoreDig == 1) {
        scoreVis[i][p].r = scoreVis[i + 1][p + 1].r = scoreVis[i + 1][p].r =
            scoreVis[i][p + 1].r = 255;
        scoreVis[i][p].g = scoreVis[i + 1][p + 1].g = scoreVis[i + 1][p].g =
            scoreVis[i][p + 1].g = 255;
        scoreVis[i][p].b = scoreVis[i + 1][p + 1].b = scoreVis[i + 1][p].b =
            scoreVis[i][p + 1].b = 255;

        livesVis[i][p].r = livesVis[i + 1][p + 1].r = livesVis[i + 1][p].r =
            livesVis[i][p + 1].r = 255;
        livesVis[i][p].g = livesVis[i + 1][p + 1].g = livesVis[i + 1][p].g =
            livesVis[i][p + 1].g = 255;
        livesVis[i][p].b = livesVis[i + 1][p + 1].b = livesVis[i + 1][p].b =
            livesVis[i][p + 1].b = 255;
      }
  ...
  Obstacle tubeSide1(75, 25, 0,
                     50); // tube on left side of screen for enemies and coins
  Obstacle tubeTop1(25, 50, 0,
                    50); // tube on left side of screen for enemies and coins
  Obstacle tubeSide2(75, 25, 975,
                     50); // tube on right side of screen for enemies and coins
  Obstacle tubeTop2(25, 50, 945,
                    50); // tube on right side of screen for enemies and coins
  ...
  // obstacle array
  Obstacle platforms[6] = {leftPlat1, rightPlat1, midPlat,
                           topLeft,   topRight,   bottomPlat};
  ...
  while (!g.getQuit()) // ONE FRAME OF THE LOOP
  {
    ...
    for (int i = 0; i < numEnemies; i++) {
      logic::Rect marioRect{mario.getPosX(), mario.getPosY(), mario.getWidth(),
                            mario.getHeight()};
      logic::Rect enemyRect{enemies[i].getPosX(), enemies[i].getPosY(),
                            enemies[i].getWidth(), enemies[i].getHeight()};
      logic::EnemyCollisionType collisionType =
          logic::classifyPlayerEnemyCollision(marioRect, enemyRect);

      if (collisionType == logic::EnemyCollisionType::stomp && doneKilling &&
          !enemies[i].getDead()) {
        kill(enemies[i]);
        logic::onEnemyStomp(gameState);
        deadEnemies = gameState.level.deadEnemies;
        score = gameState.score;
        mario.setGravity(false);
        mario.addPosY(-100);
        mario.drawMarioPixelArt(g, marioPixelJump, facingRight);
        cout << "\nEnemy killed! " << numEnemies - deadEnemies
             << " remaining.\n";
        doneKilling = false;

      }
      // kill mario
      else if (collisionType == logic::EnemyCollisionType::side &&
               !enemies[i].getDead()) {
        mario.setPosX(WINDOW_WIDTH / 2);
        mario.setPosY(30);
        logic::onPlayerHit(gameState);
        marioLives = gameState.lives;
    ...
    for (int i = 0; i < numCoins; i++) {
      if (mario.collidesWith(coins[i]) && !coins[i].getDead()) {
        logic::onCoinCollected(gameState);
        score = gameState.score;
        kill(coins[i]);
        cout << "\nYou got a coin" << endl;
      }
    ...
    if (deadEnemies == numEnemies) {
      int currentEnemyCount = numEnemies;
      // bring all enemies back to life
      reviveAll(enemies, currentEnemyCount);
      logic::onLevelCleared(gameState);
      level = gameState.level.level;
      deadEnemies = gameState.level.deadEnemies;
      numEnemies = gameState.level.enemiesToSpawn;
      coin1.setDead(true);
      if (level < 6) {
